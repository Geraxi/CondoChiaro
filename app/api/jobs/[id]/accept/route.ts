...
export async function POST(
  ...
  try {
    ...
    if (job.status !== 'pending') {
      return apiErrorResponse('Job cannot be accepted', 'INVALID_STATE', 400)
    }
    ...
    if (updateError) {
      ...
      return apiErrorResponse('Failed to accept job', 'DB_ERROR', 500)
    }

    return apiSuccessResponse(updatedJob, 'Job accepted successfully')
  } catch (error: any) {
    console.error('Accept job error:', error)
    return apiErrorResponse(error.message || 'Failed to accept job', 'SERVER_ERROR', 500)
  }
}
